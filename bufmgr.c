			...
			bool *foundPtr)
{
	...
	// changes by neha: remove usage count
	/*buf->usage_count = 0; */
...
PinBuffer(volatile BufferDesc *buf)
{
	...
	{
		// changes made by neha:calling the function implemented in freelist.c
		StrategyFreelistBuffer(buf);
		LockBufHdr(buf);
		buf->refcount++;
...
SyncOneBuffer(int buf_id, bool skip_pinned)
{
	...
	if(skip_pinned && (bufHdr->refcount != 0))
	{
		UnlockBufHdr(bufHdr);
		return false;
	}
...
ReleaseBuffer(Buffer buffer)
{
	...
	if (BufferIsLocal(buffer))
	{
		...
		bufHdr = &LocalBufferDescriptors[-buffer - 1];
		LocalRefCount[-buffer - 1]--;
		// change by neha: remove usage count
		/*if (LocalRefCount[-buffer - 1] == 0 &&
			bufHdr->usage_count < BM_MAX_USAGE_COUNT)
			bufHdr->usage_count++;
		*/
