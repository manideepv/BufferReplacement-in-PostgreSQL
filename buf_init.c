...
InitBufferPool(void)
{
	...
	else
	{
		...
		for (i = 0; i < NBuffers; buf++, i++)
		{
			...
			buf->flags = 0;
			/*
			changes by neha  -- ignoring usage_count as it is not used in lru
			*/
			//buf->usage_count = 0;
			buf->refcount = 0;
			...
			buf->freeNext = i + 1;
						/*
			changes by neha  -- For new head, we decrement the previous counter
			*/
			buf->freePrev = i - 1;
		...
		BufferDescriptors[NBuffers - 1].freeNext = FREENEXT_END_OF_LIST;
			/*
			changes by neha  -- The double linkedlist's first entry is corrected.
			*/
		BufferDescriptors[0].freePrev = FREEPREV_END_OF_LIST;
